...
[**Documentation**](https://antiartificial.github.io/contextdb) | [**Quick Start**](https://antiartificial.github.io/contextdb/quick-start) | [**v0.103.0 Recap**](https://antiartificial.github.io/contextdb/releases/v0.103.0) | [**GraphQL**](https://antiartificial.github.io/contextdb/api/graphql) | [**Feature Matrix**](https://antiartificial.github.io/contextdb/feature-matrix)

**Current release:** `v0.103.0`. contextdb is usable today, but still pre-1.0 while the public API, deployment modes, and SDK contracts continue to settle.
...
**[Explain-rank](docs/api/rest.md#explain-rank)** -- Compare two nodes and see which score factors and support-chain evidence made one rank above the other. *Typical vector DBs: rank explanations require separate observability code.*
...
**[Source trust timeline](docs/api/rest.md#source-trust-timeline)** -- Source credibility changes can be inspected over time from durable feedback events, and review tasks can be emitted for credibility drops or repeated refutations. *Typical vector DBs: source trust history is outside the retrieval system.*
...
**[Admin UI](docs/api/rest.md#admin-ui)** -- Built-in dashboard on the observe port with ranking evaluation, metrics, baseline deltas, score components, and a belief debugger for source, support, contradiction, provenance, confidence evidence, and explain-rank comparison. *Typical vector DBs: external tooling.*
...
Generate or validate the expected manifest entry from the CLI:
...
contextdb norn publish --endpoint "$CONTEXTDB_URL" --publish-url "$NORN_PUBLISH_URL" --report
```
