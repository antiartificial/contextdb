...
func TestNamespace_ReviewDecisionPersistsWorkflowState(t *testing.T) {
	...
	receipts, err := ns.ReviewHandoffDeliveryReceipts(ctx, start)
	is.NoErr(err)
	is.Equal(len(receipts), 1)
	is.Equal(receipts[0].DigestEventID, recordedDigest.EventID)
	is.Equal(receipts[0].TargetURL, webhookServer.URL)
	is.True(receipts[0].Success)
	is.Equal(receipts[0].StatusCode, http.StatusAccepted)
	is.Equal(receipts[0].PayloadSHA256, executed[0].PayloadSHA256)
	...
	is.Equal(fatigue[0].Owners, []client.ReviewHandoffRetryOwnerCount{{Owner: "alice", Count: 1}})
	is.Equal(fatigue[0].EscalationLevels, []client.ReviewHandoffRetryEscalationCount{{EscalationLevel: "review_overdue", Count: 1}})
	...
	is.True(strings.Contains(markdown, "alice=1"))
	...
	retry, err := ns.ReviewHandoffWebhookRetry(ctx, client.ReviewHandoffRetryRequest{
		After:         start,
		DigestEventID: recordedDigest.EventID,
		TargetURL:     failingServer.URL,
		Secret:        "test-secret",
		Execute:       true,
		Timeout:       time.Second,
	...
	is.NoErr(err)
	is.True(retry.Executed)
	is.Equal(retry.StatusCode, http.StatusAccepted)
	is.Equal(retry.ResponseBody, "retry accepted")
	retryCandidates, err = ns.ReviewHandoffRetryCandidates(ctx, start)
	is.NoErr(err)
	is.Equal(len(retryCandidates), 0)
