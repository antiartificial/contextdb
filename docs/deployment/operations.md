...
For graph-derived session context values, use the [KV derivation recipes](/deployment/kv-derivation-recipes) to choose stable hot-key names, labels, and review steps before executing a refresh. The stale derived KV repair recipe starts from doctor's `recommended_repair_command`, preserves the first run as a dry-run report, executes only after payload review, can write a `--receipt-out` audit file, verifies that receipt with `contextdb repair kv-cache receipt verify`, and reruns doctor to confirm freshness.
