...
Built-in presets are `review-overdue`, `source-trust-anomaly`, and `unassigned-review-overdue`. JSON responses include the same preset metadata so clients can render the available lanes without hard-coding descriptions or assembling example filters. SDK and REST tests validate that payload against `testdata/fixtures/retry-fatigue-presets.schema.json`, keeping the dashboard-facing contract stable. Docs consumers can fetch the same contract from [`/schemas/retry-fatigue-presets.schema.json`](/schemas/retry-fatigue-presets.schema.json), or discover published contracts through [`/schemas/index.json`](/schemas/index.json).
