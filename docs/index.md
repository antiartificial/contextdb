...
::: tip Current release
contextdb is currently `v0.35.0`: usable for serious demos and early integrations, but intentionally pre-1.0 while the public API, deployment modes, and SDK contracts continue to settle.
:::
...
| [Backup runbook](deployment/backup-runbook) | Scheduled export, artifact manifest verification, restore rehearsal, promotion receipts, receipt verification, lifecycle summary verification, retention reporting, delete-plan output, manifest indexes, verification, and diffs, backup marker, doctor, and Norn check flow |
