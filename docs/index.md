...
::: tip Current release
contextdb is currently `v0.108.0`: usable for serious demos and early integrations, but intentionally pre-1.0 while the public API, deployment modes, and SDK contracts continue to settle.
:::
...
| [Claim review queue](api/rest#claim-review-queue) | Ranked operator tasks with durable workflow decisions and escalation metadata |
...
| [Backup runbook](deployment/backup-runbook) | Scheduled export, artifact manifest verification, restore rehearsal, promotion receipts, receipt verification, lifecycle summary verification, retention reporting, delete-plan output, manifest indexes, verification, diffs, and metadata publish, backup marker, doctor, and Norn check flow |
...
| [Admin UI](api/rest#admin-ui) | Ranking evaluation, metrics, source-trust timelines, and belief debugger on observe port |
...
| [Knowledge acquisition planner](api/rest#knowledge-acquisition-plan) | Prioritized research, crawl, verification, and refresh tasks from gaps and weak claims, with dry-run-first connector execution and provider adapters |
