...
## Completed In v0.91.0

| Feature | Status | Evidence |
|:--------|:-------|:---------|
| Admin metrics dashboard | Implemented | `/admin/` now surfaces health signals, ingest admission/rejection rates, retrieval error rates, latency snapshots, raw `/admin/api/metrics` JSON, and the debugger/search workflow in one dashboard |

## Completed In v0.92.0
...
## Completed In v0.96.0

| Feature | Status | Evidence |
|:--------|:-------|:---------|
| KV derived refresh receipt verifier | Implemented | `contextdb repair kv-cache receipt verify --receipt PATH --value-file PATH --report` validates derived KV refresh receipt structure, embedded executed report evidence, doctor command drift, and optional reviewed value SHA-256 |

## Completed In v0.97.0

| Feature | Status | Evidence |
|:--------|:-------|:---------|
| Retry fatigue preset schema publication | Implemented | `docs/public/schemas/retry-fatigue-presets.schema.json` publishes the preset contract for docs consumers, and `TestPublicRetryFatiguePresetSchemaMatchesFixture` prevents drift from the embedded fixture |

## Completed In v0.98.0
